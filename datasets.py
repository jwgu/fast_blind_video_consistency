...
#----------------------------------
class MultiFramesDatasetScanNetDepth(data.Dataset):

    def __init__(self, opts, mode):
        super(MultiFramesDatasetScanNetDepth, self).__init__()
        ...
        for dataset, task in self.dataset_task_list:
            ...
            for video in videos:
                self.task_videos.append([task, video])
                
                input_dir = os.path.join(self.opts.data_dir, self.mode, "input", dataset, video)
                frame_list = glob.glob(os.path.join(input_dir, '*.color.jpg'))
                if len(frame_list) == 0:
                    raise Exception("No frames in %s" %input_dir)
    ...
    def __getitem__(self, index):
        ...
        frame_p = []
        for t in range(T, T + self.opts.sample_frames):
            frame_i.append( utils.read_img(os.path.join(input_dir, 'scannet', video, "frame-%06d.color.jpg" %(5*t)) ) )
            frame_p.append( utils.read_depth_img(os.path.join(process_dir, video, "frame-%06d.depth.pgm" %(5*t)) ) )
        ...
        if self.mode == 'train':
            ...
            if self.opts.geometry_aug:
                ...
                for t in range(self.opts.sample_frames):
                    frame_i[t] = cv2.resize(frame_i[t], (W_out, H_out))
                    frame_p[t] = cv2.resize(frame_p[t], (W_out, H_out))
                    frame_p[t] = np.expand_dims(frame_p[t], axis=2) # keep 3rd dimension
